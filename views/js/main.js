...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var values = document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(values[0], size);
    var newwidth = (values[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < values.length; i++) {
        values[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  var each  =  document.body.scrollTop / 1200;
  var items =  document.getElementsByClassName('mover');
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(each + (i % 5));
